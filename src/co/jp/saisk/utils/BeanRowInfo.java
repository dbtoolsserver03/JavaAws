...
public class BeanRowInfo {
	...
	public String outputInfo() {
		return "↑↑↑ outputInfo [rowNum=" + rowNum + ", caseNo=" + caseNo + ", isOk=" + isOk + ", aMapFilter=" + aMapFilter
	...
	public void doFilefillter() throws Throwable {
		...
		for (Entry<String,String> entryPath: aPathMap.entrySet()) {
			for (Entry<String,Set<String>> entryFile : aFileMap.entrySet()){
				Map<String,String> fileNmMap = getFileFilter(entryFile.getValue(),fileNmAReg);
				aMapFilter.put(entryPath.getKey(), fileNmMap);
				aFileCnt+=fileNmMap.size();
			}
		}
		for (Entry<String,String> entryPath: bPathMap.entrySet()) {
			for (Entry<String,Set<String>> entryFile : bFileMap.entrySet()){
				Map<String,String> fileNmMap = getFileFilter(entryFile.getValue(),fileNmBReg);
	...
	public void initPathBean() throws Throwable {
		aPathMap = this.isAFolderAws?AwsS3Utils.getPathSetLike(awsPathA, subPathAReg,commonKeyFolder):MyDirectory.getFolderSetLike(folderPathA,subPathAReg,commonKeyFolder);
		bPathMap = this.isBFolderAws?AwsS3Utils.getPathSetLike(awsPathB, subPathBReg,commonKeyFolder):MyDirectory.getFolderSetLike(folderPathB,subPathBReg, commonKeyFolder);
	}
	...
	public void initFileMap() {
		for (Entry<String,String> entry: aPathMap.entrySet()) {
			Set<String> fileASet = isAFolderAws?AwsS3Utils.getNameSetByS3Path(entry.getValue(), false) : MyDirectory.getFileSet(entry.getValue(), false);
			aFileMap.put(entry.getKey(), fileASet);
		}
		for (Entry<String,String> entry: bPathMap.entrySet()) {
			Set<String> fileASet = isBFolderAws?AwsS3Utils.getNameSetByS3Path(entry.getValue(), false) : MyDirectory.getFileSet(entry.getValue(), false);
